...
void lcd_init(void)
{
    ...
    // HD44780 spec init: send 0x30 (8-bit mode) three times
...
void lcd_gotoxy(int col, int row)
{
    ...
    switch (row)
    {
        case 0: address = 0x00 + col; break; // row 0 base: 0x00
        case 1: address = 0x40 + col; break; // row 1 base: 0x40
...
// Print initial message
{
    ...
    delay_ms(1000);
    lcd_cmd(0x01); // Clear
    lcd_print(" Use the buttons");
    lcd_gotoxy(0, 1);
    lcd_print("to navigate menu");
    delay_ms(1500);
    //lcd_cmd(0x01); // Clear
      ...
      // Beeping 
       ...
       // LCD logic
        if(second_flag)
        {
            ...
            // If lcd_timer_on => show time
            if(lcd_timer_on)
            {
                display_time_on_lcd();
            }

            // We also handle menu updates once per second
            switch(current_menu)
            {
                case 1:
                    lcd_timer_on = true;
